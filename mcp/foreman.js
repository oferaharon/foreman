 ...
 * error. A `lead` has fifteen other tools that never touch a pane, so the same fact
...
const SESSION_TOOLS = [
  ...
  },
  {
    ...
    handler: async (args) =>
       ...
       * posting as another session. There is no speaker argument here either, for the
       * same reason: which of the two shapes a post takes is decided by which endpoint
       * composed it, never by the caller.
       *
  ...
  },
  {
    ...
    handler: async (args) => {
      /*
       * Membership before anything is read, the way `plan_read` checks the task row first:
