...
import { FORMULA, BREW_LAUNCHD_LABEL } from '../server/homebrew.js';
...
/*
 * ------------------------------------------------------- the *other* launchd label ---
 *
 * There are two, one per install, and they are not interchangeable.
 *
 * A checkout's plist is generated by `install-agent.js` from `AGENT_LABEL` above. A
 * Homebrew install's plist is generated by `brew services` from the formula's `service do`
 * block, in another repository, and launchd names it `homebrew.mxcl.<formula>` — so under
 * Homebrew `dev.foreman.panel.plist` names no file on disk at all. `backup-state.sh` asked
 * for it anyway and reported MISSING with nothing else on the run looking wrong.
 *
...
test('the Homebrew launchd label is Homebrew’s prefix plus our one name, and is not the checkout label', () => {
  assert.equal(BREW_LAUNCHD_LABEL, `homebrew.mxcl.${FORMULA}`);
  assert.equal(BREW_LAUNCHD_LABEL, 'homebrew.mxcl.foreman-panel');
  assert.notEqual(BREW_LAUNCHD_LABEL, DEFAULT_AGENT_LABEL, 'two installs, two plists, two names');
});
...
test('the brew-label change did not touch the log basenames — that is the cascade it exists to avoid', () => {
  ...
  assert.equal(path.basename(LOG_ERR), 'foreman-error.log');
  assert.ok(
    !LOG_OUT.includes('homebrew.mxcl'),
    'the log names come from AGENT_LABEL, never from the brew service label',
  );
});
...
test('backup-state.sh reads both labels off the modules that own them, spelling neither name twice', () => {
  ...
  assert.ok(sh.includes('BREW_LAUNCHD_LABEL'), 'backup-state.sh does not read the brew launchd label');
  assert.ok(
    !sh.includes(`"${BREW_LAUNCHD_LABEL}"`) && !sh.includes(`'${BREW_LAUNCHD_LABEL}'`),
    'the composed brew label must not be a second literal in the shell — compose it from $BREW_FORMULA',
  );
...
/** Run one backup and hand back the plist path it reported, plus whether it found it. */
async function backupPlistLine(script, { home, prefix, ...overrides }) {
  ...
  const line = stdout.split('\n').find((l) => l.includes('LaunchAgent plist'));
  assert.ok(line, `no LaunchAgent line in the report:\n${stdout}`);
  ...
  return { captured: line.includes('[captured]'), plist: path.basename(found[1]), archive };
}
...
test('under a Homebrew prefix the backup names the plist brew services really wrote; outside one, the checkout’s', async (t) => {
  ...
  fs.writeFileSync(plistFor(BREW_LAUNCHD_LABEL), '<plist>brew services generated this</plist>');

  const archived = (a) => execFileSync('tar', ['-xzOf', a, './launchagent.plist'], { encoding: 'utf8' });

  const brew = await backupPlistLine(brewScript, { home, prefix });
  assert.equal(brew.plist, `${BREW_LAUNCHD_LABEL}.plist`);
  ...
  assert.match(archived(brew.archive), /brew services generated this/);
  ...
  assert.match(archived(plain.archive), /checkout/);
});
...
test('a Homebrew install with server/homebrew.js unreadable still names a brew plist, not the checkout one', async (t) => {
  const dir = fs.realpathSync(scratch(t));
  const prefix = path.join(dir, 'opt', 'brewsim');
  ...
  const { plist } = await backupPlistLine(script, { home, prefix });
  assert.equal(plist, `${BREW_LAUNCHD_LABEL}.plist`, 'the bash fallback composes the same label from $BREW_FORMULA');
});
