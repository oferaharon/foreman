...
import { FORMULA, BREW_LAUNCHD_LABELS, brewPlistPaths, existingBrewPlists } from '../server/homebrew.js';

const [BREW_LABEL_NEW, BREW_LABEL_OLD] = BREW_LAUNCHD_LABELS;
...
/*
 * ------------------------------------------------------ the *other* launchd labels ---
 *
 * There are two kinds, one per install, and they are not interchangeable — and on the
 * Homebrew side there are two *spellings*, because Homebrew renamed its own prefix.
 *
 * A checkout's plist is generated by `install-agent.js` from `AGENT_LABEL` above: one file,
 * one name. A Homebrew install's plist is generated by `brew services` from the formula's
 * `service do` block, in another repository, and launchd names it after the formula —
 * `sh.brew.<formula>` on current Homebrew, `homebrew.mxcl.<formula>` on older ones. So
 * under Homebrew `dev.foreman.panel.plist` names no file on disk at all; `backup-state.sh`
 * asked for it anyway and reported MISSING with nothing else on the run looking wrong, and
 * asking for one brew spelling on a Mac that uses the other produces the identical
 * nothing. Hence a pair, tried newest first, resolved by which file is really there —
 * never by a version number, which an install predating the rename would answer wrongly.
 *
 * The fix is the *label* and only the label. The log basenames stay derived from
 * `AGENT_LABEL` in both installs — the formula's `service do` block spells `foreman.log` /
 * `foreman-error.log`, and a brew-aware `AGENT_LABEL` would have the panel trimming two
 * files nobody writes while the ones launchd appends to grow without bound. The two tests
 * at the top of this file that pin the basenames are what stops that drifting.
 */

test('the Homebrew launchd labels are both of Homebrew’s prefixes plus our one name, newest first', () => {
  assert.deepEqual(BREW_LAUNCHD_LABELS, [`sh.brew.${FORMULA}`, `homebrew.mxcl.${FORMULA}`]);
  assert.deepEqual(BREW_LAUNCHD_LABELS, ['sh.brew.foreman-panel', 'homebrew.mxcl.foreman-panel']);
  // Order is the contract, not an accident of how the array was typed: current Homebrew
  // writes the first, so a caller that takes one takes the one most installs have.
  assert.equal(BREW_LABEL_NEW, 'sh.brew.foreman-panel');
  for (const label of BREW_LAUNCHD_LABELS) {
    assert.notEqual(label, DEFAULT_AGENT_LABEL, 'two installs, two kinds of plist, no shared name');
  }
});

test('the brew-label change did not touch the log basenames — that is the cascade it exists to avoid', () => {
  // Said again here, next to the label, because this is the assertion somebody "tidying"
  // the two labels into one would have to delete on purpose.
  assert.equal(path.basename(LOG_OUT), 'foreman.log');
  assert.equal(path.basename(LOG_ERR), 'foreman-error.log');
  for (const label of BREW_LAUNCHD_LABELS) {
    const brewPrefix = label.slice(0, -FORMULA.length); // `sh.brew.` / `homebrew.mxcl.`
    assert.ok(
      !LOG_OUT.includes(brewPrefix),
      'the log names come from AGENT_LABEL, never from a brew service label',
    );
  }
});

/*
 * The resolver, against a scratch HOME. Four states and all four are real: neither plist
 * (a checkout, or a formula never started), only the new spelling (a current install),
 * only the old one (an install that predates the rename and has never been re-`start`ed),
 * and both (an upgrade that wrote the new one without removing the old). The last is the
 * one a single-answer resolver would get wrong quietly, by capturing half of what is there.
 */
test('existingBrewPlists answers by what is on disk — none, either one, or both', (t) => {
  const home = scratch(t);
  const agents = path.join(home, 'Library', 'LaunchAgents');
  fs.mkdirSync(agents, { recursive: true });
  const plistFor = (label) => path.join(agents, `${label}.plist`);

  assert.deepEqual(brewPlistPaths(home), [plistFor(BREW_LABEL_NEW), plistFor(BREW_LABEL_OLD)]);
  assert.deepEqual(existingBrewPlists(home), [], 'nothing on disk is not a plist to capture');

  fs.writeFileSync(plistFor(BREW_LABEL_NEW), '<plist>current homebrew</plist>');
  assert.deepEqual(existingBrewPlists(home), [plistFor(BREW_LABEL_NEW)]);

  fs.rmSync(plistFor(BREW_LABEL_NEW));
  fs.writeFileSync(plistFor(BREW_LABEL_OLD), '<plist>older homebrew</plist>');
  assert.deepEqual(existingBrewPlists(home), [plistFor(BREW_LABEL_OLD)], 'the old spelling is still a real install');

  fs.writeFileSync(plistFor(BREW_LABEL_NEW), '<plist>current homebrew</plist>');
  assert.deepEqual(
    existingBrewPlists(home),
    [plistFor(BREW_LABEL_NEW), plistFor(BREW_LABEL_OLD)],
    'both present is an upgrade that did not clean up — capture both, newest first',
  );

  // A directory named like a plist is not a plist: `isFile`, never `existsSync`.
  fs.rmSync(plistFor(BREW_LABEL_OLD));
  fs.mkdirSync(plistFor(BREW_LABEL_OLD));
  assert.deepEqual(existingBrewPlists(home), [plistFor(BREW_LABEL_NEW)]);
});

test('backup-state.sh reads both labels off the modules that own them, spelling neither name twice', () => {
  const sh = fs.readFileSync(path.resolve(import.meta.dirname, '..', 'scripts', 'backup-state.sh'), 'utf8');

  assert.ok(
    sh.includes('$SCRIPT_DIR/../server/homebrew.js'),
    'backup-state.sh should resolve server/homebrew.js relative to its own directory, as it does logs.js',
  );
  assert.ok(
    sh.includes('brewPlistPaths') && sh.includes('existingBrewPlists'),
    'backup-state.sh does not read the brew plist resolvers off the module that owns them',
  );
  for (const label of BREW_LAUNCHD_LABELS) {
    assert.ok(
      !sh.includes(`"${label}"`) && !sh.includes(`'${label}'`),
      `the composed brew label ${label} must not be a second literal in the shell — compose it from $BREW_FORMULA`,
    );
  }
  // Both spellings have to survive in the bash fallback, or a copy of this script carried
  // off without node beside it goes back to knowing exactly one of them.
  for (const label of BREW_LAUNCHD_LABELS) {
    const prefix = label.slice(0, -FORMULA.length);
    assert.ok(
      sh.includes(`${prefix}$BREW_FORMULA.plist`),
      `the bash fallback does not compose the ${prefix} spelling from $BREW_FORMULA`,
    );
  }
  // The formula name's bash copy, held to the module that owns it. `test/cli.test.js` holds
  // the third copy (package.json's `bin` key) to the same constant.
  assert.ok(
    sh.includes(`BREW_FORMULA="${FORMULA}"`),
    `backup-state.sh spells a different formula name than server/homebrew.js (${FORMULA})`,
  );
});

/*
 * …and then the behaviour, because every assertion above is about the *text* of a shell
 * script and none of them would notice the branch being wired up backwards.
 *
 * Two copies of the package: one under a path shaped like a Homebrew prefix, one outside
 * it. Both plists exist in a scratch HOME, so which one the run captures is a fact read
 * off the archive rather than inferred from the absence of the other. Nothing here touches
 * a real prefix, a real plist, the real state dir or the running panel: HOME, the prefix
 * and the port are all scratch, and `--force` means the live-panel probe never decides
 * anything.
 */

/** A package copy — `server/`, `scripts/`, `package.json` — rooted at `dest`. */
function packageCopy(dest) {
  const repo = path.resolve(import.meta.dirname, '..');
  fs.mkdirSync(dest, { recursive: true });
  for (const item of ['server', 'scripts', 'package.json']) {
    fs.cpSync(path.join(repo, item), path.join(dest, item), { recursive: true });
  }
  return path.join(dest, 'scripts', 'backup-state.sh');
}

/**
 * Run one backup and hand back every plist line it reported.
 *
 * A list rather than one line, because a Homebrew install can genuinely have two plists and
 * the whole question here is whether the run finds all of them. `plist` and `captured` stay
 * as they were for the single-plist cases, read off the first line.
 */
async function backupPlistLine(script, { home, prefix, ...overrides }) {
  const env = { ...process.env, HOME: home, HOMEBREW_PREFIX: prefix, FOREMAN_PORT: '54871', ...overrides };
  for (const key of ['FOREMAN_STATE_DIR', 'FOREMAN_AGENT_LABEL', 'FOREMAN_LOG_DIR']) {
    if (!(key in overrides)) delete env[key];
  }
  const { stdout } = await run('bash', [script, '--force'], { env });
  const lines = stdout.split('\n').filter((l) => l.includes('LaunchAgent plist'));
  assert.ok(lines.length, `no LaunchAgent line in the report:\n${stdout}`);
  const plists = lines.map((line) => {
    // A captured line ends with the size (`… .plist 39 B`), a missing one with the path —
    // so match the path itself rather than taking the last field.
    const found = line.match(/(\S+\.plist)/);
    assert.ok(found, `no plist path on the LaunchAgent line: ${line}`);
    return { captured: line.includes('[captured]'), plist: path.basename(found[1]) };
  });
  // The archive path off the report, not by listing the directory: the filename is
  // second-resolution, so two runs in one second are one file.
  const archive = stdout.split('\n').find((l) => l.startsWith('Archive: '))?.slice('Archive: '.length).trim();
  return { ...plists[0], plists, archive, stdout };
}

/** The plist basenames really inside an archive, in the order tar lists them. */
function archivedPlists(archive) {
  return execFileSync('tar', ['-tzf', archive], { encoding: 'utf8' })
    .split('\n')
    .map((entry) => entry.replace(/^\.\//, ''))
    .filter((entry) => entry.startsWith('launchagents/') && entry.endsWith('.plist'))
    .map((entry) => path.basename(entry));
}

/** The bytes of one plist inside an archive, by label. */
function archivedPlist(archive, label) {
  return execFileSync('tar', ['-xzOf', archive, `./launchagents/${label}.plist`], { encoding: 'utf8' });
}

test('under a Homebrew prefix the backup names the plist brew services really wrote; outside one, the checkout’s', async (t) => {
  // realpath, because the script compares its own *physical* directory against the prefix
  // as given — and macOS's temp dir is a symlink.
  const dir = fs.realpathSync(scratch(t));

  const prefix = path.join(dir, 'opt', 'brewsim');
  const brewScript = packageCopy(path.join(prefix, 'Cellar', 'foreman-panel', '0.2.0', 'libexec', 'lib', 'node_modules', 'foreman'));
  const plainScript = packageCopy(path.join(dir, 'elsewhere', 'foreman'));

  // Both plists on disk: the pick has to be a choice, not the only file there.
  const home = path.join(dir, 'home');
  fs.mkdirSync(path.join(home, 'Library', 'LaunchAgents'), { recursive: true });
  const plistFor = (label) => path.join(home, 'Library', 'LaunchAgents', `${label}.plist`);
  fs.writeFileSync(plistFor(DEFAULT_AGENT_LABEL), '<plist>checkout</plist>');
  fs.writeFileSync(plistFor(BREW_LABEL_NEW), '<plist>brew services generated this</plist>');

  const brew = await backupPlistLine(brewScript, { home, prefix });
  assert.equal(brew.plist, `${BREW_LABEL_NEW}.plist`);
  assert.equal(brew.captured, true, 'the brew plist is there and should have been captured');
  // And the archive holds the *bytes*, not just a path that reads right.
  assert.deepEqual(archivedPlists(brew.archive), [`${BREW_LABEL_NEW}.plist`]);
  assert.match(archivedPlist(brew.archive, BREW_LABEL_NEW), /brew services generated this/);

  const plain = await backupPlistLine(plainScript, { home, prefix });
  assert.equal(plain.plist, `${DEFAULT_AGENT_LABEL}.plist`, 'a copy outside the prefix is not a Homebrew install');
  assert.equal(plain.captured, true);
  assert.deepEqual(archivedPlists(plain.archive), [`${DEFAULT_AGENT_LABEL}.plist`]);
  assert.match(archivedPlist(plain.archive, DEFAULT_AGENT_LABEL), /checkout/);
});

/*
 * The rename, from the backup's end. Homebrew writes `sh.brew.<formula>` now and wrote
 * `homebrew.mxcl.<formula>` before, and an install of either age is a live install whose
 * plist has to end up in the archive. The both-present case is the one worth having: a
 * resolver that stopped at its first hit would pass every other test here and quietly
 * capture half of what is on the Mac.
 */
test('either brew spelling on its own is captured, and both together are both captured', async (t) => {
  const dir = fs.realpathSync(scratch(t));
  const prefix = path.join(dir, 'opt', 'brewsim');
  const script = packageCopy(path.join(prefix, 'Cellar', 'foreman-panel', '0.4.0', 'libexec'));
  ...
  // (a) only the current spelling
  fs.writeFileSync(plistFor(BREW_LABEL_NEW), '<plist>written by current homebrew</plist>');
  const onlyNew = await backupPlistLine(script, { home, prefix });
  assert.deepEqual(archivedPlists(onlyNew.archive), [`${BREW_LABEL_NEW}.plist`]);
  assert.equal(onlyNew.captured, true);
  assert.match(archivedPlist(onlyNew.archive, BREW_LABEL_NEW), /written by current homebrew/);

  // (b) only the older spelling — an install that predates the rename is still an install
  fs.rmSync(plistFor(BREW_LABEL_NEW));
  fs.writeFileSync(plistFor(BREW_LABEL_OLD), '<plist>written by older homebrew</plist>');
  const onlyOld = await backupPlistLine(script, { home, prefix });
  assert.deepEqual(archivedPlists(onlyOld.archive), [`${BREW_LABEL_OLD}.plist`]);
  assert.equal(onlyOld.captured, true);
  assert.match(archivedPlist(onlyOld.archive, BREW_LABEL_OLD), /written by older homebrew/);

  // (c) both — an upgrade that wrote the new plist and left the old one behind
  fs.writeFileSync(plistFor(BREW_LABEL_NEW), '<plist>written by current homebrew</plist>');
  const both = await backupPlistLine(script, { home, prefix });
  assert.deepEqual(
    archivedPlists(both.archive).sort(),
    [`${BREW_LABEL_NEW}.plist`, `${BREW_LABEL_OLD}.plist`].sort(),
    'both spellings are on disk, so both belong in the archive',
  );
  assert.equal(both.plists.length, 2, 'the report names each plist it captured');
  assert.ok(both.plists.every((p) => p.captured));
  assert.match(archivedPlist(both.archive, BREW_LABEL_NEW), /written by current homebrew/);
  assert.match(archivedPlist(both.archive, BREW_LABEL_OLD), /written by older homebrew/);

  // (d) neither — the run has to name what it went looking for, both spellings of it,
  //     rather than report one MISSING path and leave the other unmentioned.
  fs.rmSync(plistFor(BREW_LABEL_NEW));
  fs.rmSync(plistFor(BREW_LABEL_OLD));
  const neither = await backupPlistLine(script, { home, prefix });
  assert.equal(neither.plists.length, 2);
  assert.ok(neither.plists.every((p) => !p.captured));
  assert.deepEqual(
    neither.plists.map((p) => p.plist),
    [`${BREW_LABEL_NEW}.plist`, `${BREW_LABEL_OLD}.plist`],
    'newest spelling named first',
  );
});

test('FOREMAN_AGENT_LABEL renames the checkout’s plist and leaves the brew service’s alone', async (t) => {
  const dir = fs.realpathSync(scratch(t));
  const prefix = path.join(dir, 'opt', 'brewsim');
  const brewScript = packageCopy(path.join(prefix, 'Cellar', 'foreman-panel', '0.2.0', 'libexec'));
  const plainScript = packageCopy(path.join(dir, 'elsewhere'));
  const home = path.join(dir, 'home');
  fs.mkdirSync(path.join(home, 'Library', 'LaunchAgents'), { recursive: true });

  const scratchLabel = 'com.example.foreman-scratch';
  const plain = await backupPlistLine(plainScript, { home, prefix, FOREMAN_AGENT_LABEL: scratchLabel });
  assert.equal(plain.plist, `${scratchLabel}.plist`, 'the bench-isolation contract still holds for a checkout');

  // The variable renames the plist *install-agent.js* writes. `brew services` has never
  // heard of it, so honouring it there would send the backup after a file that cannot exist.
  const brew = await backupPlistLine(brewScript, { home, prefix, FOREMAN_AGENT_LABEL: scratchLabel });
  assert.equal(brew.plist, `${BREW_LABEL_NEW}.plist`);
});

test('a Homebrew install with server/homebrew.js unreadable still names both brew plists, not the checkout one', async (t) => {
  const dir = fs.realpathSync(scratch(t));
  const prefix = path.join(dir, 'opt', 'brewsim');
  const pkg = path.join(prefix, 'Cellar', 'foreman-panel', '0.2.0', 'libexec');
  const script = packageCopy(pkg);
  fs.rmSync(path.join(pkg, 'server', 'homebrew.js'));

  const home = path.join(dir, 'home');
  const agents = path.join(home, 'Library', 'LaunchAgents');
  fs.mkdirSync(agents, { recursive: true });

  // Nothing on disk: the fallback has to *name* both spellings, which is what proves it
  // carries the pair rather than whichever one somebody last hardcoded.
  const missing = await backupPlistLine(script, { home, prefix });
  assert.deepEqual(
    missing.plists.map((p) => p.plist),
    [`${BREW_LABEL_NEW}.plist`, `${BREW_LABEL_OLD}.plist`],
    'the bash fallback composes both labels from $BREW_FORMULA',
  );

  // And it has to find the older one, which is the spelling a copy of this script carried
  // off on its own is most likely to meet.
  fs.writeFileSync(path.join(agents, `${BREW_LABEL_OLD}.plist`), '<plist>older homebrew</plist>');
  const found = await backupPlistLine(script, { home, prefix });
  assert.equal(found.plist, `${BREW_LABEL_OLD}.plist`);
  assert.equal(found.captured, true);
  assert.deepEqual(archivedPlists(found.archive), [`${BREW_LABEL_OLD}.plist`]);
});
